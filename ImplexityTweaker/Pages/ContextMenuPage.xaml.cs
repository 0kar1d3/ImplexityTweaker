...
public partial class ContextMenuPage : Page
{
    ...
    private void Refresh()
    {
        ...
        try
        {
            ...
            OldContextMenuToggle.IsChecked = s.OldContextMenu ?? (key != null);
        }
        catch
        {
            OldContextMenuToggle.IsChecked = s.OldContextMenu ?? false;
        }
        ...
        try
        {
            ...
            RemoveContextDelayToggle.IsChecked = s.RemoveContextDelay ?? (v == 0);
        }
        catch
        {
            RemoveContextDelayToggle.IsChecked = s.RemoveContextDelay ?? false;
        }
        ...
        try
        {
            ...
            OpenInTerminalToggle.IsChecked = s.OpenInTerminal ?? (key != null);
        }
        catch
        {
            OpenInTerminalToggle.IsChecked = s.OpenInTerminal ?? false;
        }

        // Дополнительные (нереализованные) переключатели — восстанавливаем из конфига.
        ShareToggle.IsChecked = s.Share ?? false;
        RestoreOldVersionToggle.IsChecked = s.RestoreOldVersion ?? false;
        SendToggle.IsChecked = s.Send ?? false;
        CopyPathToggle.IsChecked = s.CopyPath ?? false;
        PinStartToggle.IsChecked = s.PinStart ?? false;
        PinTaskbarToggle.IsChecked = s.PinTaskbar ?? false;
    }
    ...
    private async void Apply_Click(object sender, System.Windows.RoutedEventArgs e)
    {
        ...
        // Сохраняем состояние всех переключателей в конфиг.
        var s = AppSettings.Load();
        s.OldContextMenu = oldToggle;
        s.RemoveContextDelay = delayToggle;
        s.OpenInTerminal = termToggle;
        s.Share = ShareToggle.IsChecked == true;
        s.RestoreOldVersion = RestoreOldVersionToggle.IsChecked == true;
        s.Send = SendToggle.IsChecked == true;
        s.CopyPath = CopyPathToggle.IsChecked == true;
        s.PinStart = PinStartToggle.IsChecked == true;
        s.PinTaskbar = PinTaskbarToggle.IsChecked == true;
        s.Save();
