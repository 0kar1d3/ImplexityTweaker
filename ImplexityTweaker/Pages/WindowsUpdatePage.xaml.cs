...
public partial class WindowsUpdatePage : Page
{
    public WindowsUpdatePage()
    {
        ...
        if (!string.IsNullOrEmpty(s.WuStartupMode))
        {
            foreach (ComboBoxItem item in StartupModeBox.Items)
            {
                if (item.Content?.ToString() == s.WuStartupMode)
                {
                    StartupModeBox.SelectedItem = item;
                    break;
                }
            }
        }
        if (StartupModeBox.SelectedItem == null)
