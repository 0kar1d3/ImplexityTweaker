...
public partial class TelemetryPage : Page
{
    ...
    private void ApplyAllSafeDefaults()
    {
        ...
        // Если в конфиге задано состояние — применяем его, иначе безопасные значения по умолчанию.
        TogglePolicyTelemetry.IsChecked = s.TelemetryPolicy ?? true;
        ToggleDiagTrack.IsChecked = s.TelemetryDiagTrack ?? true;
        ToggleDmwappush.IsChecked = s.TelemetryDmwappush ?? true;
        ToggleCompatAppraiser.IsChecked = s.TelemetryCompatAppraiser ?? true;
        ToggleDeviceCensus.IsChecked = s.TelemetryDeviceCensus ?? true;
        ToggleAdvertisingId.IsChecked = s.TelemetryAdvertisingId ?? true;
        ToggleTailoredExperiences.IsChecked = s.TelemetryTailoredExperiences ?? true;
        ToggleStartSuggestions.IsChecked = s.TelemetryStartSuggestions ?? true;
        ToggleFeedbackFrequency.IsChecked = s.TelemetryFeedbackFrequency ?? true;
        ToggleWerConsent.IsChecked = s.TelemetryWerConsent ?? true;
    }
    ...
    private async Task RunBuildAsync(bool onlyChecked)
    {
        ...
        var s = AppSettings.Load();
        s.TelemetryPolicy = TogglePolicyTelemetry.IsChecked == true;
        s.TelemetryDiagTrack = ToggleDiagTrack.IsChecked == true;
        s.TelemetryDmwappush = ToggleDmwappush.IsChecked == true;
        s.TelemetryCompatAppraiser = ToggleCompatAppraiser.IsChecked == true;
        s.TelemetryDeviceCensus = ToggleDeviceCensus.IsChecked == true;
        s.TelemetryAdvertisingId = ToggleAdvertisingId.IsChecked == true;
        s.TelemetryTailoredExperiences = ToggleTailoredExperiences.IsChecked == true;
        s.TelemetryStartSuggestions = ToggleStartSuggestions.IsChecked == true;
        s.TelemetryFeedbackFrequency = ToggleFeedbackFrequency.IsChecked == true;
        s.TelemetryWerConsent = ToggleWerConsent.IsChecked == true;
        s.Save();
